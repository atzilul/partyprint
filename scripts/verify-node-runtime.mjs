...
try{
 ...
 await login();assert.equal((await request('/admin')).status,200);const adminBlog=await request('/api/admin/blog');assert.equal(adminBlog.status,200);assert((await adminBlog.json()).posts.length>=18);
