...
Owner-only settings maintain one versioned shirtPrice. Every package and additional shirt uses quantity × shirtPrice, rounded using integer agorot. Derived legacy fields are output compatibility only, never independent pricing inputs. Legacy settings convert using the family package unit price; the initial default is 112.25 NIS, preserving four shirts at 449 NIS. Public totals and order creation fetch this price book. Existing orders keep their saved amount. Price changes while a form is open cause a review prompt rather than accepting an outdated total.
