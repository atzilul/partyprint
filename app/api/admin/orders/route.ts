...
export async function GET(request:Request){
 ...
 try{
  ...
  return Response.json({orders:rows.results.map(r=>({...normalizeOrder(JSON.parse(r.data)),version:r.version})),total:count?.n||0,counts:counts.results,mailReady:!mailConfiguration({RESEND_API_KEY,MAIL_FROM}).issue,pendingMail:pending?.n||0,attention:{overdue:attention?.overdue||0,today:attention?.today||0},today},{headers:PRIVATE_HEADERS});
