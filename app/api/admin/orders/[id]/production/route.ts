...
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}){
 ...
 try{
  ...
  const o=await getOrder((await params).id);if(!o)return new Response(null,{status:404,headers:PRIVATE_HEADERS});if(v?.version!==o.version)throw Error('conflict');
  if(v.action==='approval'){
   ...
   const token=Array.from(crypto.getRandomValues(new Uint8Array(32))).map(b=>b.toString(16).padStart(2,'0')).join(''),id=crypto.randomUUID(),images=[];
   ...
   const approval={id,hash:await tokenHash(token),expiresAt:Date.now()+30*86400000,state:'pending' as const,createdAt:new Date().toISOString(),sourceKeys:keys,images,shirtsSignature:shirtSignature(o),quantity:o.quantity,shirts:o.shirts};
   const saved=await saveOrder({...o,approval,status:'review'},o.version!,'נוצרה גרסת עיצוב לאישור לקוח. יש לשתף את הקישור.','מנהל');committed=true;
