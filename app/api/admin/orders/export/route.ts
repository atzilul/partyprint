import {paidTotal} from '@/lib/production';
...
export async function GET(request:Request){
 ...
 try{
  ...
  const records=[['מספר הזמנה','לקוח','טלפון','מייל','שלב','סוג פנייה','חבילה','כמות','סכום הזמנה בשקלים','התקבל נטו','יתרה לתשלום','מועד יעד','כתובת','מעקב','קופון','הנחה מקורית','נוצרה','עודכנה'],...rows.results.map(r=>{const o=normalizeOrder(JSON.parse(r.data));return [o.id,o.name,o.phone,o.email,stageLabel(o.status),o.mode==='lead'?'התייעצות':'הזמנה',o.packageName,o.quantity,o.price,paidTotal(o),Math.round((o.price-paidTotal(o))*100)/100,o.dueDate,o.address,o.tracking,o.couponCode,o.discount||0,o.createdAt,o.updatedAt]})];
