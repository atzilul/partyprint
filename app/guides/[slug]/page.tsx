...
export async function generateMetadata({params}: {params: Promise<{slug: string}>}): Promise<Metadata> {
  ...
  return {title: item.seoTitle, description: item.seoDescription, keywords: item.keywords, alternates: {canonical: SEO_PAGE_URL(item.slug)}, openGraph: {title: item.seoTitle, description: item.seoDescription, type: 'article', images: [{url: item.heroImage, alt: item.heroAlt}]} };
}
...
export default async function SeoGuidePage({params}: {params: Promise<{slug: string}>}) {
  ...
  const pageSchema = {'@context': 'https://schema.org', '@type': 'WebPage', name: item.title, description: item.seoDescription, url: canonicalUrl, primaryImageOfPage: {'@type': 'ImageObject', url: `https://partyprint.co.il${item.heroImage}`, caption: item.heroAlt}};
