...
export default function BlogIndex({posts}:{posts:BlogPost[]}){
 ...
 return <>
  ...
  {featured&&<article className="blog-feature"><Link className="blog-feature-image" href={'/blog/'+featured.slug}><img src={featured.coverImage} sizes="(max-width: 800px) 100vw, 55vw" alt={featured.coverAlt} loading="eager" fetchPriority="high" decoding="async"/></Link><div className="blog-feature-copy"><span className="blog-category">הכתבה המומלצת</span><h2>{featured.title}</h2><p>{featured.excerpt}</p><Link href={'/blog/'+featured.slug}>לקריאת המדריך <span aria-hidden="true">←</span></Link></div></article>}
...
function BlogCard({post}:{post:BlogPost}){return <article className="blog-card"><Link className="blog-card-image" href={'/blog/'+post.slug}><img src={post.coverImage} sizes="(max-width: 520px) 100vw, (max-width: 800px) 50vw, 33vw" alt={post.coverAlt} loading="lazy" decoding="async"/></Link><div className="blog-card-body"><div className="blog-card-meta"><span className="blog-category">{post.keywords[0]||'מדריך PARTYPRINT'}</span><span>{Math.max(3,Math.ceil(post.content.split(/\s+/).length/180))} דקות</span></div><h2>{post.title}</h2><p>{post.excerpt}</p><Link className="blog-card-link" href={'/blog/'+post.slug}>למדריך המלא <span aria-hidden="true">↙</span></Link></div></article>}
