...
import {DEFAULT_PRICING,Pricing,Coupon} from './pricing';
export async function getPricing(){const r=await bindings().DB.prepare("SELECT data,version FROM studio_settings WHERE key='pricing'").first<{data:string;version:number}>();return r?{...JSON.parse(r.data),version:r.version} as Pricing:DEFAULT_PRICING}
