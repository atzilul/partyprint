...
export type Order={subtotal?:number;discount?:number;couponCode?:string;id:string;name:string;phone:string;email:string;brief:string;package:number;packageName:string;quantity:number;price:number;mode:string;createdAt:string;updatedAt?:string;status:string;priceStatus:string;shirts:{size:string;color:string}[];images:OrderImage[];version?:number;notes?:string;address?:string;dueDate?:string;tracking?:string;revisions?:number;priceNote?:string;emailStatus?:string;history?:{at:string;action:string;actor:string}[]};
...
export function editOrder(old:Order,v:Record<string,unknown>,pricing:Pricing=DEFAULT_PRICING):Order{
 ...
 const name=str('name',100),phone=str('phone',20),email=str('email',150),brief=str('brief',3000),notes=str('notes',5000),address=str('address',500),tracking=str('tracking',500),dueDate=str('dueDate',10),priceNote=str('priceNote',500);
 ...
 return {...old,name,phone,email,brief,notes,address,tracking,dueDate,priceNote,package:p,packageName:packs[p].name,quantity,price,revisions,status:String(v.status),mode:String(v.mode),shirts:v.mode==='lead'?[]:v.shirts as Order['shirts'],priceStatus:price!==old.price?'adjusted':old.priceStatus};
}
