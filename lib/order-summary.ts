...
type SummaryOrder={eventDate?:string;subtotal?:number;discount?:number;couponCode?:string;address?:string;tracking?:string;dueDate?:string;revisions?:number;id:string;name:string;phone:string;email?:string;brief:string;packageName:string;quantity:number;price:number;mode:string;createdAt:string;priceStatus:string;shirts:{size:string;color:string}[];images:{name:string}[]};
...
...(o.couponCode?[`קוד מבצע בעת ההזמנה: ${o.couponCode}`,`הנחה שניתנה: ${o.discount||0} ₪`]:[]),'כולל עיצוב משותף אחד, הדפסה ומשלוח.',...(o.address?[`כתובת למשלוח: ${o.address}`]:[]),...(o.eventDate?[`תאריך האירוע שביקש הלקוח (לא אישור אספקה): ${o.eventDate}`]:[]),...(o.dueDate?[`מועד יעד שסוכם: ${o.dueDate}`]:[]),...(o.tracking?[`מעקב משלוח: ${o.tracking}`]:[]),'',
...
'עד 5 ימי עסקים למשלוח לאחר אישור העיצוב, ההזמנה והתשלום, ובכפוף לאישור המועד. זמן העיצוב אינו כלול.',
